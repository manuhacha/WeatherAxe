...
export class HourlyWeather {
  ...
  protected readonly hourlyWeather = signal<HourlyWeatherData[]>([]);
  ...
  loadWeekly() {
    ...
    const hourlyForecast: HourlyWeatherData[] = data.hourly.time.map((time, i) => {
      const weather = this.weatherCodeService.getWeatherCode(data.hourly.weather_code[i], data.hourly.is_day[i]);
      return {
      time,
